...
def get_iou(box1, box2, epsilon=1e-5):
    ...
    iou = intersection / (union_area + epsilon)
    return iou
...
def plot_one_box(x, img, color=None, label=None, line_thickness=None):
    ...
    if label:        
        if '\n' in label:
            ...
            # c2 = c1[0] + int(t_size[0]/2), c1[1] - int(t_size[1]*2.5) - 3
            # cv2.rectangle(img, c1, c2, color, -1)  # filled
            for i, line in enumerate(label.split('\n')):
                print(line)
                t_size = cv2.getTextSize(line, 0, fontScale=tl / 3, thickness=tf)[0]
                c2 = c1[0] + t_size[0], c1[1] - t_size[1] - 3
                cv2.rectangle(img, (c1[0], c1[1]-50 + 50*i), (c2[0], c2[1] - 50 + 50*i), color, -1)  # filled
                cv2.putText(img, line, (c1[0], c1[1] - 55 + 50*i), 0, tl / 3, [225, 255, 255], thickness=tf, lineType=cv2.LINE_AA)
        else:
            tf = max(tl - 1, 1)  # font thickness
            t_size = cv2.getTextSize(label, 0, fontScale=tl / 3, thickness=tf)[0]
            c2 = c1[0] + t_size[0], c1[1] - t_size[1] - 3
            cv2.rectangle(img, c1, c2, color, -1)  # filled
            cv2.putText(img, label, (c1[0], c1[1] - 5), 0, tl / 3, [225, 255, 255], thickness=tf, lineType=cv2.LINE_AA)
